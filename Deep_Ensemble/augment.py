...
class Augmentation:
    ...
    def random_rotate(trajs, origin=None):
        ...
        for i in range(trajs.shape[0]):
            ...
            xy_ = trajs[i, :, :2].T
            uv_ = trajs[i, :, 2:].T
            rotated[i, :, :2] = np.matmul(rot_mat, xy_ - orig).T
            rotated[i, :, 2:] = np.matmul(rot_mat, uv_).T
        return rotated
    ...
    def swap_xyuv(trajs):
        ...
        y-coordinates as well as the x- and y-velocity of the trajectories.
        
        Assumption: trajs is a numpy array where:
        - trajs[:, :, 0] and trajs[:, :, 1] are the x and y positions
        - trajs[:, :, 2] and trajs[:, :, 3] are the u (x velocity) and v (y velocity) components
